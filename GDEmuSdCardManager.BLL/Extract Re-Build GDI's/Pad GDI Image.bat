...
Echo  Padding image please wait...
..\tools\gditools.exe -i "..\%~nx1\%GDI%" --data-folder "Extracted" -b "Extracted\bootsector\IP.BIN" --extract-all --silent
..\tools\buildgdi.exe -data "..\%~nx1\Extracted" -ip "..\%~nx1\Extracted\bootsector\IP.BIN" -output "..\%~nx1" -gdi "%GDI%" -raw >NUL
...
:Cleanup
:: Clean up, remove Extracted folder & none needed files.
Set /p Total=<"%GDI%"
Set /a Total+=1
If %Total% LSS 10 Set "TRACK=TRACK0%Total%.bin"
If %Total% GTR 9 Set "TRACK=TRACK%Total%.bin"
if exist "%TRACK%" del /Q "%TRACK%"
RD /S /Q "Extracted"
Goto End
